...
// @license		   GPL
// @credits		   Great Thanks for suggestion, encouragement, testing to Mario Heiderich. Wordlists taken from  WSfuzzer, Acunetix. Sorry for redundancy. JS Colorizer by Cezary Tomczak, http://code.gosu.pl
...
unsafeWindow.yehgBackupFileSearch = function()
{
	...
	unsafeWindow.yehgcurrentjob = "Backfile Scaning: " + thisURI; 
...
if(unsafeWindow.document.getElementsByTagName("form")[f].elements[e]!= null && document.getElementsByTagName("form")[f].elements[e]!= "undefined")
      {        
            ...
            {
                formsdata += "No. " + e + " . Name: " +  unsafeWindow.document.getElementsByTagName("form")[f].elements[e].name+"&nbsp;&nbsp;[type=" +unsafeWindow.document.getElementsByTagName("form")[f].elements[e].type+"]&nbsp;&nbsp;[value=<span style='color:red;font-weight:bold'>" + escape(unsafeWindow.document.getElementsByTagName("form")[f].elements[e].value) + "</span>]";
            ...
            {
                formsdata += "No. " + e + " . Name: " +  unsafeWindow.document.getElementsByTagName("form")[f].elements[e].name+"&nbsp;&nbsp;[type=" +unsafeWindow.document.getElementsByTagName("form")[f].elements[e].type+"]&nbsp;&nbsp;[value=" + escape(unsafeWindow.document.getElementsByTagName("form")[f].elements[e].value) + "]<br>";
...
for(var s=0;s<=unsafeWindow.document.getElementsByTagName("script").length-1;s++)
{
    ...
    {
        scriptsrc += 'No. ' + s + ' <a  target="_blank"  style="color:yellow!important;text-decoration:underline;" href="' + unsafeWindow.document.getElementsByTagName("script")[s].src + '?fingerprint">'+unsafeWindow.document.getElementsByTagName("script")[s].src+'</a><br>';      
    }
    else
    {
...
for(w in unsafeWindow.window)
{
    if( (!nativeObj.test(w)) && (!yehgObj.test(w)))
    {
        if(typeof(eval("unsafeWindow."+w)) == "object")
        {
            if(w.indexOf("mm")==0)
            {
                cusObj  += "=>"+w+"&nbsp;&nbsp;&nbsp;&nbsp; " + eval("unsafeWindow."+w)+"&nbsp;&nbsp; {possible source: adobe products}<br>";    
            }
            else if(w.indexOf("google_")==0 || w.indexOf("_u")==0 || w.indexOf("gaGlobal")==0)
            {
                cusObj  += "=>"+w+"&nbsp;&nbsp;&nbsp;&nbsp; " + eval("unsafeWindow."+w)+"&nbsp;&nbsp; {possible source: google ads}<br>";    
            }
            else
            {
...
for(w in unsafeWindow.window)
{
    if( (!nativeObj.test(w)) && (!yehgObj.test(w)))
    {
        if(typeof(eval("unsafeWindow."+w)) == "function")
        {
            ...
            {
                cusObj  += "=><a style='color:yellow!important;text-decoration:underline;' href='javascript:alert("+w+")'>" + w + "()</a>&nbsp;&nbsp; {possible source: google ads}<br>";
            }
